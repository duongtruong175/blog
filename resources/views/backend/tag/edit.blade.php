    ...
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white sm:rounded-lg">
        <form method="POST" action="{{ route('backend_tag.update', $tag->id) }}">
            ...
            <div class="mt-4">
                ...
                <input class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" id="name" name="name" value="{{ $tag->name }}" required autofocus onfocus="this.selectionStart = this.value.length;" />
            </div>
